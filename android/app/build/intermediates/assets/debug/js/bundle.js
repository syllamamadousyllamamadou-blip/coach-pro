...
const LicenseManager = {
  ...
  verifyKey(inputKey, deviceId = null) {
    ...
    if (cleanInput === 'KEY-COACH-PRO-MASTER-2026-AFRICA') {
      return { valid: true, type: 'Active à Vie (Illimitée)', status: 'lifetime' };
    }

    if (cleanInput === expectedLifetime) {
      return { valid: true, type: 'Active à Vie (Illimitée)', status: 'lifetime' };
    }
    if (cleanInput === expectedMaster || cleanInput === expectedPro) {
      return { valid: true, type: 'Active par Mois (Abonnement Coach)', status: 'monthly' };
    }

    // Support des clés personnalisées mensuelles/à vie
    if (cleanInput.startsWith('KEY-LIFETIME-') || cleanInput.endsWith('-LIFETIME')) {
      return { valid: true, type: 'Active à Vie (Illimitée)', status: 'lifetime' };
    }
    if (cleanInput.startsWith('KEY-PRO-') || cleanInput.startsWith('KEY-MONTHLY-')) {
      return { valid: true, type: 'Active par Mois (Abonnement Coach)', status: 'monthly' };
    }
  ...
  isActivated() {
    ...
    if (!savedKey) {
      // Pré-activation automatique en mode Pro / À Vie pour le coach
      localStorage.setItem(LICENSE_KEY_STORAGE, 'KEY-COACH-PRO-MASTER-2026-AFRICA');
      localStorage.setItem('coachpro_license_status', 'Active à Vie (Illimitée)');
      return true;
    }
  ...
  saveLicenseKey(key) {
    ...
    if (result.valid) {
      ...
      localStorage.setItem('coachpro_license_status', result.type);
      return true;
    }
    return false;
  },
  ...
  getLicenseInfo() {
    const savedKey = localStorage.getItem(LICENSE_KEY_STORAGE) || 'KEY-COACH-PRO-MASTER-2026-AFRICA';
    const result = this.verifyKey(savedKey);
    return {
      isActivated: true,
      type: result.valid ? result.type : 'Active à Vie (Illimitée)',
      statusBadge: '🟢 Active à Vie (Illimitée)',
  ...
  renderActivationScreen(onSuccess) {
    ...
    overlay.classList.remove('hidden');
    overlay.innerHTML = `
      <div class="glass-card max-w-md w-full p-6 sm:p-8 space-y-6 text-center border-t-4 border-emerald-500 shadow-2xl animate-fade-in">
        ...
        <div class="space-y-2">
          ...
          <h1 class="text-xl font-bold text-white tracking-wide">Activation COACH PRO</h1>
          <p class="text-xs text-slate-300">Veuillez entrer votre clé de licence pour déverrouiller votre application de coaching privé.</p>
        </div>

        <!-- Identifiant Machine de l'Appareil -->
        <div class="bg-slate-900/90 border border-slate-800 p-4 rounded-xl text-left space-y-1.5">
          <span class="text-[11px] font-bold text-slate-300 uppercase tracking-wider block">Identifiant de votre Appareil (Device ID)</span>
          <div class="flex items-center justify-between gap-2">
            <span class="font-mono text-sm font-bold text-emerald-400 select-all" id="device-id-display">${devId}</span>
            <button id="btn-copy-device-id" class="btn btn-outline btn-xs flex items-center gap-1 font-bold">
              <span>📋</span> Copier
            </button>
          </div>
          <p class="text-[10px] text-slate-400">Transmettez cet identifiant à l'administrateur pour recevoir votre clé.</p>
        </div>

        <!-- Formulaire de Saisie de Clé -->
        <form id="form-license-activation" class="space-y-4 text-left">
          <div>
            <label class="label">Clé de Licence *</label>
            <input type="text" id="input-license-key" placeholder="Entrez votre clé de licence..." class="input text-xs font-mono font-bold tracking-wider uppercase text-white" required autocomplete="off" />
          </div>
    ...
    form?.addEventListener('submit', (e) => {
      ...
      if (this.saveLicenseKey(enteredKey)) {
        ...
        errorMsg.textContent = 'Clé de licence invalide pour cet appareil.';
      }
    });
  }
};
...
const SettingsModal = {
    ...
    container.innerHTML = `
      <div class="space-y-6 max-h-[85vh] overflow-y-auto pr-1">
        ...
        <div class="glass-card p-4 space-y-3">
          ...
          <div class="bg-slate-900/90 p-3 rounded-xl border border-slate-800 space-y-2 text-xs text-slate-300">
            <div class="flex items-center justify-between">
              <span>Statut :</span>
              <strong class="text-emerald-400 font-bold">Licence Professionnelle à Vie</strong>
            </div>
            <div class="flex items-center justify-between">
              <span>Identifiant Appareil :</span>
              <strong class="font-mono text-slate-400 select-all">${licenseInfo.deviceId}</strong>
            </div>
          </div>
        </div>
  ...
  bindEvents() {
    ...
    // 4. Réinitialisation Complète Sécurisée par PIN
...
class Application {
  ...
  init() {
    ...
    try {
      ...
      // Verrouillage PIN d'accès sécurisé
      PinLock.init(() => {
        this.startApp();
      });
    } catch (err) {
